...
- **Foreign keys are on, and `delete_person` deletes children explicitly anyway.** D1 enforces foreign keys, and this design depends on cascades for `delete_person` and `purge_roster_source`. An earlier revision of this spec asserted that cascaded deletes do not fire the `AFTER DELETE` triggers maintaining the FTS indexes, reasoning from SQLite's documented statement that foreign key actions are unaffected by the recursive-triggers setting. **That inference was wrong and is withdrawn.** The sentence means those actions happen regardless of the setting, not that they bypass triggers, and a test on 2026-08-24 against SQLite 3.51 with `foreign_keys = ON` and `recursive_triggers = OFF` confirmed a cascaded child delete removes its FTS row. `delete_person` still deletes children explicitly, in application code, inside the same batch, for weaker reasons that are worth stating rather than dressing up: D1 runs its own SQLite build inside workerd and nobody has checked it, and this is the one tool that exists to satisfy erasure requests. **The guarantee comes from the test asserting no FTS row survives a hard delete, not from the cascades and not from the explicit deletes.**
...
- A hard-delete test that asserts **no FTS row survives**. This is the only thing that actually guarantees a deleted person is out of the search index - not the cascades, and not the explicit child deletes either. The symptom it catches is silent: a deleted person still appearing in search, months later, in the one tool that exists to answer an erasure request.
