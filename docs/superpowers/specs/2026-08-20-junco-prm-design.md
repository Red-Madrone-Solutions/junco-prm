...
**Where the chunk cap comes from: measurement, not arithmetic.** Two earlier drafts derived it from D1's query budget, on the reading that each statement inside a `db.batch()` spends one of the 50 queries a free-plan invocation is allowed. That derivation is probably wrong and is withdrawn. Cloudflare's own pages disagree with each other: the Workers limits page lists 50 **external** subrequests against 1,000 subrequests to internal services on the free plan, and D1 is an internal service, while D1's limits page prints 50 for queries per invocation. `batch()` is documented as sending its statements "inside a single call to the database."

The constraint that probably does bind is the one the free plan enforces hardest and this spec has been quietest about: **10 ms of CPU per invocation.** A chunk parses rows, validates each one, builds canonical JSON, and computes a SHA-256 per row for `external_row_key` and another for `content_hash`. Several hundred digests inside 10 ms is the thing that fails, and it fails as the runtime killing the invocation mid-chunk rather than as an error the protocol can report.

So the cap is an empirical constant, and the first implementation task measures it before any import code exists:

- a `db.batch()` of 49, 50, and 60 trivial statements on a free-plan Worker, which settles the query-counting question by observation;
- CPU milliseconds per row for parse, canonical JSON, and two digests, giving rows per 10 ms;
- and, in the same deploy, whether a `[[ratelimits]]` binding is accepted on a free account at all.

The cap is the smaller of the two row counts, halved for margin, and the spec records the measurement that produced it. Until then, 150 is a placeholder chosen to be safe under either reading, not a derived number.
...
None that block implementation. Three things are unverified rather than undecided, and all three are settled by one stub deploy on a free Cloudflare account before the import code is written:

1. **Does a `db.batch()` of N statements spend N of the free plan's per-invocation query budget, or one?** Cloudflare's Workers limits page distinguishes 50 external subrequests from 1,000 to internal services, D1 is an internal service, and D1's own limits page prints 50. Measured by running batches of 49, 50, and 60.
2. **How much CPU does a roster row cost** to parse, canonicalize, and hash twice? The chunk cap comes out of this against a 10 ms budget.
3. **Is the Workers rate-limiting binding available on the free plan?** Its documentation does not say. If the deploy refuses it, the fallback is a KV token bucket.

The remaining uncertainty is not a question but a condition: nothing here has been run. The claims about D1 behavior, FTS5 under D1, and the deploy sequence are researched rather than observed, and the first execution of plan 1 is what turns them into facts.
